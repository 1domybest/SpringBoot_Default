package com.example.Default_Project.filter;

import com.example.Default_Project.config.SecurityConfig;

import com.example.Default_Project.entity.UserEntity;
import com.example.Default_Project.jwt.CustomUserDetails;
...
/**
 * 토큰 검증 클래스
 * @see SecurityConfig 에서 사용하는 JWT 토큰 검증 요청 필터
 */
...
public class JWTFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        // 토큰이 없다면 다음 필터로 넘김
        if (request.getRequestURI().equals("/login") || request.getRequestURI().equals("/oauth2") || request.getRequestURI().equals("/token-refresh") || request.getRequestURI().equals("/env")) {
